...
2. **学习曲线合理**：你有 C/C++ 基础，Python 上手极快；前端从 React 组件化开始，不需要深入 CSS 底层。
3. **展示技术深度**：不是简单的调用 API，而是涉及数据解析、Prompt Engineering、流式输出、数据库设计、前后端联调等完整工程链路。
4. **真实可演示**：你可以上传一个真实的销售数据 CSV 文件，平台自动生成分析报告和图表，面试官可以当场体验。
5. **与 Kimi 结合**：你使用 Kimi API 作为核心 AI 能力，可以展示你对大模型应用开发的深入理解。

---

> **下一步：查看 `plan.md` 获取详细的 14 天开发计划，我们将一步步推进项目！** 🚀
